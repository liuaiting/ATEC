...
#                 f1 = float(f1_score(target, pred))
#                 plot_accs.append(accuracy)
#                 sys.stdout.write(
#                     '\rBatch[{}-{}] - loss: {:.6f}  acc: {:.6f}  f1: {:.6f}  time: {}s'.format(
#                         epoch, steps, loss.item(), accuracy, f1, time.time()-start_time))
...
#         avg_loss += loss.item()
#
#         pred = torch.argmax(logit, dim=1).tolist()
...
#     avg_loss /= size
#     print('\nEvaluation - loss: {:.6f}  acc: {:.6f}   f1: {:.6f}  time: {}s\n'.format(
...
def predict(inpath, outpath, model, sentence_field, id_field, cuda_flag):
    ...
    for batch in predict_iter:
        ...
        idx = batch.id.tolist()
        predict_label = torch.argmax(logit, dim=1).tolist()
