...
def process(inpath, outpath="test_xgb.csv"):
    ...
    print(dtest.num_col())
    bst = xgb.Booster(model_file="xgb_model/model/0005.model")  # init model
    ypred = bst.predict(dtest)

    ypred = pd.DataFrame(list(map(lambda x: int(round(x)), ypred)))
    xgb_df = pd.concat([ids, ypred], axis=1)
    xgb_df.to_csv(outpath, sep=str("\t"), header=False, index=False, encoding="utf-8")
    xgb_df = pd.read_csv(outpath, sep="\t", header=None, names=["idx", "pred1"])
    print(xgb_df.head())
